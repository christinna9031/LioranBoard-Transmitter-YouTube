...
function LBLoadTabsUI() {
  const tabList = {};
  let tabSortList = JSON.parse(localStorage.getItem('tabsSortList_yt')) || [];
  const newtabSortList = [];
  const tabsVisibility = JSON.parse(localStorage.getItem('tabsVisibility_yt')) || [];
  let lastActiveTab = localStorage.getItem('tabsActive_yt') || 'content-status';
  lastActiveTab = (document.getElementById(lastActiveTab)) ? lastActiveTab : 'content-status'
  ...
  if (activeTab) activeTab.className = 'tab-pane active';
  ...
  localStorage.setItem('tabsSortList_yt', JSON.stringify(newtabSortList));
  ...
  draggable.on('sortable:sorted', (e) => {
    const sortArr = JSON.parse(localStorage.getItem('tabsSortList_yt'));
    sortArr.splice(e.newIndex, 0, sortArr.splice(e.oldIndex, 1)[0]);
    localStorage.setItem('tabsSortList_yt', JSON.stringify(sortArr));
  });
  ...
  document.querySelector('#extensionsshow').onclick = () => {
    localStorage.removeItem('tabsVisibility_yt');
    location.reload();
  };
  ...
  document.querySelector('#extensionshide').onclick = () => {
    ...
    localStorage.setItem('tabsVisibility_yt', JSON.stringify(tabsVisiblity));
    window.location.reload();
  };

  // reset tab order
  document.querySelector('#extensionsresetorder').onclick = () => {
    localStorage.removeItem('tabsSortList_yt');
    window.location.reload();
  };

  // save active tab
  ul.querySelectorAll('button').forEach((btn) => btn.onclick = (btn) => {
    localStorage.setItem('tabsActive_yt',btn.target.id.slice(0,-4))
  });
  ...
  function SaveExttabsVisibility() {
    const tabsVisiblity = JSON.parse(localStorage.getItem('tabsVisibility_yt')) || {};
    document.querySelectorAll('#installedextensions input[type=checkbox]').forEach((e) => {
      const id = e.id.slice(8);
